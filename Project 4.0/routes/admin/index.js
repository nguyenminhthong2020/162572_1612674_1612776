...
router.post('/edit-category/:id', (req, res) => {
    ...
    entity.then(entity => {
        ...
        rs.then(rs => {
            console.log('Update success');
            res.redirect('/admin/manage-category/all');
            res.json(200);
        }).catch(err => {
            console.log(err);
            res.redirect('/admin/manage-category/all');
...
router.get('/action-delete-post/:id', function (req, res) {
    ...
    Promise.all([cateAll, postAll]).then(values => {
        ...
        rs.then(rs => {
            ...
            res.redirect('/admin/manage-post/Published');
            res.json(200);
        }).catch(err => {
            console.log(err);
            res.redirect('/admin/manage-post/Published');
...
router.get('/edit-tag/:id', (req, res) => {
    ...
    Promise.all([tagAll]).then(values => {
        ...
        res.render("admin/editTag", { title: "edit-tag", layout: "admin.hbs", thisTag: thisTag[0], id: id });
    })
        .catch(err => console.log(err));
});
router.post('/edit-tag/:id', (req, res) => {
    ...
    entity.then(entity => {
        ...
        rs.then(rs => {
            console.log('Update success');
            res.redirect('/admin/manage-tag/all');
            res.json(200);
        }).catch(err => {
            console.log(err);
            res.redirect('/admin/manage-tag/all');
...
router.get('/manage-tag/add', (req, res) => {
    var tagAll = tagdb.getAllTag();
    Promise.all([tagAll]).then(values => {
        res.render("admin/addTag", { title: "add-tag", layout: "admin.hbs"});
    })
        .catch(err => console.log(err));
});
