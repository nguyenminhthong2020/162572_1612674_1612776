...
router.get('/', function (req, res) {
    res.redirect('/editor/manage-post/waitPublish');
});
router.get('/manage-post/history', (req, res, next) => {
    ...
    Promise.all([
        ...
        .then(values => {
            ...
            if (req.session.user)
                userName = req.session.user.name;
            //console.log(rows);
            res.render('editor/waitPublishPost', {
                layout: 'editor.hbs',
                listPost: listWaitPublish,
                fullStatus: fullStatus,
                userName: userName
    ...
    Promise.all([
        ...
        .then(values => {
            ...
            if (req.session.user)
                userName = req.session.user.name;
            //console.log(rows);
            res.render('editor/waitApprovePost', {
                layout: 'editor.hbs',
    ...
    Promise.all([
        ...
        .then(values => {
            ...
            if (req.session.user)
                userName = req.session.user.name;
            //console.log(rows);
            res.render('editor/publishedPost', {
                layout: 'editor.hbs',
...
router.get('/manage-post/add', (req, res) => {
    ...
    Promise.all([cateAll, tagAll]).then(values => {
        res.render("editor/addPost", { title: "add-post", layout: "editor.hbs", categories: values[0], tags: values[1] });
    })
...
router.post('/manage-post/add', (req, res) => {
    ...
    Promise.all([
        ...
        .then(values => {
            ...
            entity.created_by = 6; // là ID của editor
            entity.id = maxID + 1;
...
router.get('/edit-post/:id', (req, res) => {
    ...
    Promise.all([cateAll, tagAll, post, itsTag]).then(values => {
        ...
        res.render("editor/editPost", { title: "edit-post", layout: "editor.hbs", categories: values[0], itsCateName: itsCate[0].name, tags: values[1], post: values[2], id: id });
    })
        .catch(err => console.log(err));
});
...
router.get('/edit-category/:id', (req, res) => {
    ...
    Promise.all([cateAll]).then(values => {
        ...
        res.render("editor/editCategory", { title: "edit-category", layout: "editor.hbs", listParentCate: listParentCate, thisCate: thisCate[0], parent_id: parent_id, parent_name: parent_name, id: id });
    })
        .catch(err => console.log(err));
});
router.post('/edit-category/:id', (req, res) => {
    ...
    entity.then(entity => {
        ...
        rs.then(rs => {
            console.log('Update success');
            res.redirect('/editor/manage-category/all');
            res.json(200);
        }).catch(err => {
            console.log(err);
            res.redirect('/editor/manage-category/all');
...
router.get('/manage-category/add', (req, res) => {
    ...
    Promise.all([cateAll]).then(values => {
        var listParentCate = values[0].filter(x => x.parent_id == 0);
        res.render("editor/addCategory", { title: "add-category", layout: "editor.hbs", listParentCate: listParentCate });
    })
        .catch(err => console.log(err));
});
...
router.get('/action-approve-post/:id', function (req, res) {
    ...
    Promise.all([cateAll, postAll]).then(values => {
        ...
        rs.then(rs => {
            console.log('Publish success');
            res.redirect('/editor/manage-post/waitApprove');
            res.json(200);
        }).catch(err => {
            console.log(err);
            res.redirect('/editor/manage-post/waitApprove');
...
router.get('/action-delete-post/:id', function (req, res) {
    ...
    Promise.all([cateAll, postAll]).then(values => {
        ...
        rs.then(rs => {
            ...
            res.redirect('/editor/manage-post/Published');
            res.json(200);
        }).catch(err => {
            console.log(err);
            res.redirect('/editor/manage-post/Published');
...
router.get('/edit-tag/:id', (req, res) => {
    ...
    Promise.all([tagAll]).then(values => {
        ...
        res.render("editor/editTag", { title: "edit-tag", layout: "editor.hbs", thisTag: thisTag[0], id: id });
    })
        .catch(err => console.log(err));
});
router.post('/edit-tag/:id', (req, res) => {
    ...
    entity.then(entity => {
        ...
        rs.then(rs => {
            console.log('Update success');
            res.redirect('/editor/manage-tag/all');
            res.json(200);
        }).catch(err => {
            console.log(err);
            res.redirect('/editor/manage-tag/all');
...
router.get('/manage-tag/add', (req, res) => {
    var tagAll = tagdb.getAllTag();
    Promise.all([tagAll]).then(values => {
        res.render("editor/addTag", { title: "add-tag", layout: "editor.hbs"});
    })
        .catch(err => console.log(err));
});
